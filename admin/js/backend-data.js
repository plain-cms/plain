...
// from sales-analytics tracking). Both are gated by the backend's admin token,
// kept in localStorage (BYOK) and sent only to your backend — never committed.

import { h } from './ui.js';

const TOKEN_KEY = 'plain.backend.token';   // the backend's Admin:Token, shared by both views
const num = (n) => Number(n || 0).toLocaleString('en-US');
const pct = (a, b) => `${b ? Math.round((a / b) * 1000) / 10 : 0}%`;
const when = (s) => { const d = new Date(s); return isNaN(d) ? String(s || '') : d.toLocaleString(); };

// Resolve services.backend, fetch `path` with the stored bearer token (prompting for it
// on 401/403), and hand the parsed JSON to render(). h() sets strings as textContent, so
// backend data is inert in the DOM — no escaping needed.
function dataScreen(siteInfo, { title, path, hint, render }) {
  ...
  async function load() {
    ...
    let token = ''; try { token = localStorage.getItem(TOKEN_KEY) || ''; } catch { /* private mode */ }
    try {
...
export function feedbackScreen(siteInfo) {
  return dataScreen(siteInfo, {
    ...
    hint: 'This inbox is protected — paste your backend admin token (its Admin:Token) to read messages.',
    render: (data) => {
...
export function insightsScreen(siteInfo) {
  return dataScreen(siteInfo, {
    ...
    hint: 'This report is protected — paste your backend admin token (its Admin:Token) to view analytics.',
    render: (data) => {
