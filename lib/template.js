...
//
// A Mustache-flavored subset — enough, and no more:
//   {{ expr }}                 escaped output
//   {{{ expr }}}               raw output (for rendered HTML)
//   {{#if expr}} … {{else}} … {{/if}}
//   {{#each list as item}} … {{/each}}
//   {{> partial-name}}         partials, passed in as strings
//
// Expressions are dot-paths only (item.title, site.url). No arbitrary JS —
// this keeps templates safe for AI generation and trivial to parse.
//
