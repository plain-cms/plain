...
export async function loadPlugins(root, config) {
  ...
  for (const name of config.plugins) {
    ...
    if (!fs.existsSync(manifestPath)) {
      throw new ContentError(`plugins/${name}`, null, `plugin "${name}" is enabled in site.config.json but plugins/${name}/plugin.json doesn't exist — copy the plugin folder in, or remove "${name}" from "plugins"`);
    }
    let manifest;
    try {
  ...
  for (const plugin of plugins) {
    ...
    for (const [kind, file] of Object.entries(client)) {
      ...
      if (!fs.existsSync(from)) {
        throw new ContentError(`plugins/${plugin.name}/plugin.json`, null, `declares client ${kind} "${file}" but the file doesn't exist`);
      }
